...
// Projects
const projectFields = ['title', 'description', 'category', 'technologies', 'client', 'images', 'url', 'featured', 'status'];
...
const createProject = async (req, res) => { try { const data = await Project.create(pick(req.body, projectFields)); ok(res, data, 201); } catch (e) { err(res, e); } };
const updateProject = async (req, res) => { try { const data = await Project.findByIdAndUpdate(req.params.id, pick(req.body, projectFields), { new: true }); ok(res, data); } catch (e) { err(res, e); } };
...
// Services
const serviceFields = ['name', 'description', 'price', 'duration', 'features', 'category', 'isActive'];
const getServices = async (req, res) => { try { const data = await Service.find({ isActive: true }).lean(); ok(res, data); } catch (e) { err(res, e); } };
const createService = async (req, res) => { try { const data = await Service.create(pick(req.body, serviceFields)); ok(res, data, 201); } catch (e) { err(res, e); } };
const updateService = async (req, res) => { try { const data = await Service.findByIdAndUpdate(req.params.id, pick(req.body, serviceFields), { new: true }); ok(res, data); } catch (e) { err(res, e); } };
...
// Testimonials
const testimonialFields = ['client', 'company', 'text', 'rating', 'projectId', 'isPublished'];
...
const createTestimonial = async (req, res) => { try { const data = await Testimonial.create(pick(req.body, testimonialFields)); ok(res, data, 201); } catch (e) { err(res, e); } };
const updateTestimonial = async (req, res) => { try { const data = await Testimonial.findByIdAndUpdate(req.params.id, pick(req.body, testimonialFields), { new: true }); ok(res, data); } catch (e) { err(res, e); } };

// Blog
const blogFields = ['title', 'content', 'slug', 'tags', 'published', 'publishedAt', 'views', 'likes', 'coverImage', 'author'];
...
const createBlog = async (req, res) => { try { const body = pick(req.body, blogFields); if (!body.slug && body.title) body.slug = body.title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, ''); const data = await Blog.create(body); ok(res, data, 201); } catch (e) { err(res, e); } };
const updateBlog = async (req, res) => { try { const data = await Blog.findByIdAndUpdate(req.params.id, pick(req.body, blogFields), { new: true }); ok(res, data); } catch (e) { err(res, e); } };
...
// Contact Form (public)
const contactFields = ['name', 'email', 'message', 'service', 'budget'];
const submitContact = async (req, res) => { try { const data = await ContactForm.create(pick(req.body, contactFields)); ok(res, { message: 'Message received. We will contact you soon!', id: data._id }, 201); } catch (e) { err(res, e); } };
...
// Team
const teamFields = ['name', 'role', 'bio', 'skills', 'social', 'avatar', 'isActive', 'order'];
...
const createTeamMember = async (req, res) => { try { const data = await TeamMember.create(pick(req.body, teamFields)); ok(res, data, 201); } catch (e) { err(res, e); } };
const updateTeamMember = async (req, res) => { try { const data = await TeamMember.findByIdAndUpdate(req.params.id, pick(req.body, teamFields), { new: true }); ok(res, data); } catch (e) { err(res, e); } };
