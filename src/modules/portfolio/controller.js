...
const createProject = async (req, res) => { try { const data = await Project.create(pick(req.body, ['title', 'description', 'client', 'category', 'images', 'tags', 'liveUrl', 'githubUrl', 'featured', 'status'])); ok(res, data, 201); } catch (e) { err(res, e); } };
const updateProject = async (req, res) => { try { const data = await Project.findByIdAndUpdate(req.params.id, pick(req.body, ['title', 'description', 'client', 'category', 'images', 'tags', 'liveUrl', 'githubUrl', 'featured', 'status']), { new: true }); ok(res, data); } catch (e) { err(res, e); } };
...
const createService = async (req, res) => { try { const data = await Service.create(pick(req.body, ['name', 'description', 'icon', 'features', 'pricing', 'isActive'])); ok(res, data, 201); } catch (e) { err(res, e); } };
const updateService = async (req, res) => { try { const data = await Service.findByIdAndUpdate(req.params.id, pick(req.body, ['name', 'description', 'icon', 'features', 'pricing', 'isActive']), { new: true }); ok(res, data); } catch (e) { err(res, e); } };
...
const createTestimonial = async (req, res) => { try { const data = await Testimonial.create(pick(req.body, ['clientName', 'company', 'role', 'content', 'rating', 'isPublished'])); ok(res, data, 201); } catch (e) { err(res, e); } };
const updateTestimonial = async (req, res) => { try { const data = await Testimonial.findByIdAndUpdate(req.params.id, pick(req.body, ['clientName', 'company', 'role', 'content', 'rating', 'isPublished']), { new: true }); ok(res, data); } catch (e) { err(res, e); } };
...
const createBlog = async (req, res) => { try { if (!req.body.slug) req.body.slug = req.body.title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, ''); const data = await Blog.create(pick(req.body, ['title', 'content', 'slug', 'author', 'tags', 'coverImage', 'published', 'publishedAt'])); ok(res, data, 201); } catch (e) { err(res, e); } };
const updateBlog = async (req, res) => { try { const data = await Blog.findByIdAndUpdate(req.params.id, pick(req.body, ['title', 'content', 'slug', 'author', 'tags', 'coverImage', 'published', 'publishedAt']), { new: true }); ok(res, data); } catch (e) { err(res, e); } };
...
// Contact Form (public)
const submitContact = async (req, res) => { try { const data = await ContactForm.create(pick(req.body, ['name', 'email', 'phone', 'subject', 'message'])); ok(res, { message: 'Message received. We will contact you soon!', id: data._id }, 201); } catch (e) { err(res, e); } };
...
const createTeamMember = async (req, res) => { try { const data = await TeamMember.create(pick(req.body, ['name', 'role', 'bio', 'avatar', 'socialLinks', 'order', 'isActive'])); ok(res, data, 201); } catch (e) { err(res, e); } };
const updateTeamMember = async (req, res) => { try { const data = await TeamMember.findByIdAndUpdate(req.params.id, pick(req.body, ['name', 'role', 'bio', 'avatar', 'socialLinks', 'order', 'isActive']), { new: true }); ok(res, data); } catch (e) { err(res, e); } };
